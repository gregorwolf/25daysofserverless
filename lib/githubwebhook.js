...
function getEntityUrl (hash) {
	return backend + "('" + hash + "')";
}

function doesImageExist(hash) {
	return new Promise((resolve,reject) => {
		var entityUrl = getEntityUrl(hash);
...
async function uploadImage(hash, imageData) {
	var entityUrl = getEntityUrl(hash);
	var contentUrl = entityUrl + "/content"
	...
	// console.log(options)
	return await httpsCall(imageData, options)
}
	...
	doesImageExist(sha256hash).then(image => {
		...
		if(typeof(image.error) !== "undefined") {
			createImage(sha256hash).then(ret => {
				uploadImage(sha256hash, data).then(uploadres => {
